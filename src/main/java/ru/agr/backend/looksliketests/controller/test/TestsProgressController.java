...
import ru.agr.backend.looksliketests.controller.exception.StudentNotAssignedToTest;
...
public class TestsProgressController {
    ...
    public ResponseEntity<TestProgressResource> start(@TestId @PathVariable Long testId,
            ...
            throws ResourceNotFoundException, MaxTestAttemptsExceededException, StudentNotAssignedToTest {
        ...
        if (!assignationService.isStudentAssignedToTest(user.getId(), testId)) {
            throw new StudentNotAssignedToTest(userDetails.getUsername(), testId.toString());
        }
        final var attempts = testProgressService.findAllByTestIdAndUserId(testId, user.getId()).size();
